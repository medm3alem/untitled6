...
int main() {
    ...
    while (!WindowShouldClose()) {
        // CRITIQUE: UpdateMusicStream en premier
        UpdateMusicStream(jeu.music);
        ...
        // CORRECTION PROBLÈME 2: Si fin de partie online, bloquer le jeu
        if (jeu.fin_partie_online && connected) {
            std::cout << "Online game finished - disconnecting..." << std::endl;
            disconnect();
            connected = false;
        ...
        // CORRECTION PROBLÈME 2: Empêcher input et move_down si partie terminée
        if (jeu.start && !jeu.fin_partie_online) {
            jeu.input();
        }
        
        if (event(0.2/(jeu.get_niveau()+1)) && jeu.start && !jeu.fin_partie_online) {
            jeu.move_down();
        }
