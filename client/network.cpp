...
void network_connect() {
    // Lancer la connexion dans un thread séparé pour ne pas bloquer le jeu
    connection_thread = std::thread([]() {
        ...
        const char* server_ip = "127.0.0.1";  // Changez selon votre config
        ...
        // IMPORTANT: Cette opération peut prendre du temps, mais elle est dans un thread séparé
        if (connect(sock, (sockaddr*)&server, sizeof(server)) < 0) {
            std::cerr << "ERROR: Connection failed: " << strerror(errno) << std::endl;
            std::cerr << "Make sure the server is running on " << server_ip << ":4242" << std::endl;
            close(sock);
    ...
    connection_thread.detach();  // Détacher le thread
}
